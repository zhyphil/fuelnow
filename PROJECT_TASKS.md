...
> 下一项任务：用户确认重开同一项目后仍为西班牙语，本次偏好保留子项通过；下一步把已保存语言改为 English，检查即时显示与重开后是否保留新选择。首页 Combustible 单词内换行仍待优化，全页面三语及其他持久化场景未验收。地图持续空白及 Fuel 详情油品上下文仍待修复，原连接故障根因及 USB 长期稳定性未确认。iPhone LAN 测试暂不可用，App 安装仍待处理；发布条件未关闭，见 [真机测试说明](./docs/development/local-phone-testing.md)
> 最后更新：2026-09-07
