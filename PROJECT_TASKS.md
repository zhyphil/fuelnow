...
> 下一项任务：华为 USB 模式下 Charge 排序限制说明与最近排序/ETA 缺失提示已由照片确认，下一步验收 Toulouse / Air 独立搜索，再测 Wash；Charge 禁用项实际点击仍待验证。Fuel 详情油品上下文仍待修复，原连接故障根因及 USB 长期稳定性未确认。iPhone LAN 测试暂不可用，App 安装仍待处理；发布条件未关闭，见 [真机测试说明](./docs/development/local-phone-testing.md)
> 最后更新：2026-09-07
