...
> 下一项任务：本地真机测试环境已实现，用户确认 iPhone 和华为浏览器后端均可达；接着安装匹配 SDK 57 的客户端、验证 App 内连接并实际验收。华为系统仅显示 EMUI 12.0.0，底层 Android 版本暂留空，不阻塞安装尝试；发布与真实 Beta 条件仍未关闭，见 [真机测试说明](./docs/development/local-phone-testing.md)
> 最后更新：2026-09-07
