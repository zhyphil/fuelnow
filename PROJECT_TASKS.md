...
> 下一项任务：华为照片确认多服务详情、FR 内容和 DEMO 导航禁用提示显示；发现 Fuel 详情未传递列表油品上下文，需修复并回归，当前油价详情不算通过。可先独立验收列表选择 Gazole 后的报价与排序。iPhone App 安装仍待处理；发布与真实 Beta 条件仍未关闭，见 [真机测试说明](./docs/development/local-phone-testing.md)
> 最后更新：2026-09-07
