...
> 当前状态：华为使用 Toulouse 中心 12 km、79 站官方快照；真实列表/地图/柴油报价已核对。导航回调丢失 Linking 接收者的问题已修复，真机点击详情导航成功打开 Google Maps 路线预览，未启动逐向导航；同因来源链接一并修复。全量 1028 tests 通过。未自动同步、未启用付费路线、未修改原业务库。Phase 5 为 10/21 主任务及 18 个子项；Phase 6 为 5/13 主任务及 8 个准备子项；未发布，小时任务保持暂停
> 当前阶段：本机真实燃油数据有限真机测试（单次区域快照，不是全国实时服务；Phase 5/6 均未整体验收）
> 下一项任务：等待用户体验反馈；详情中营业时间原始 JSON 的可读性待处理，来源地址与地图反向地理编码门牌差异需保留边界。自动刷新策略、其他平台、实际行程及长期 USB 稳定性、生产账单治理与发布门槛未关闭。保留当前测试服务，见 [真实燃油测试说明](./docs/development/local-real-fuel-testing.md)
> 最后更新：2026-09-07
...
2026-09-07 导航修复：用户同意修复并验证打开 Google Maps；仅保留 React Native Linking 的方法接收者，沿用原 HTTPS destination-only 路线链接，不新增 API/密钥/权限或自动开始导航。同因来源/许可证链接一并修复，未扩大到来源数据格式改造。

2026-09-07 真实燃油手机测试：用户要求接入真实数据并确认“要”，采用固定 Toulouse 中心 12 km 的官方单次快照、独立临时库与回环 USB；显式 `LIVE_SOURCE_CHECK=true` 和 `--real-fuel` 才启用，默认仍 DEMO。保留原始时间与未知字段；不自动持续同步、扩区、收费路线或分析上传，不请求新 GPS 权限。

2026-09-07 地图测试包替换：用户明确同意仅卸载旧 Fuel Now Test（清除其本地偏好/测试数据）、安装专用签名地图包并测试地图。精确目标 com.fuelnow.localtest；不卸载 Expo Go，不改其他应用和系统设置，不授予真实定位权限，不启动 DEMO 外部导航。

2026-09-07 独立 Android 测试包：用户同意基础地图/标记范围和本机工具链/测试包准备，并确认尚无 Google 项目或地图密钥。安装最小 Android SDK、沿用现有 JDK 21，不安装 Studio/模拟器，不使用付费云构建。test 环境使用 Fuel Now Test / com.fuelnow.localtest，与 Expo Go 并存；结算账户、专用地图签名/密钥和地图实际验收另行确认，不启用路线计费。

2026-09-07 真机问题修复：用户明确授权开始修复，依次处理 Fuel 详情油品上下文、首页长词布局和地图空白。首页按实际可用宽度及系统 fontScale 切换单/双列，不缩小字体或截断文案；工程验证与华为人工复验分开记录，小时任务继续暂停。
...
2026-09-07 导航交接已恢复：华为 Google Maps 确实已安装，系统能够解析链接；程序传递未绑定的 Linking.openURL 导致其内部 this._validateURL 在原生调用前失败。真实路线预览通过，不等于逐向导航、iOS 或全站点现场验收。测试站官方地址为 30 Chemin de Ferro-Lèbres，Google 对相同目的地坐标显示 20 Chem. Ferro-Lèbres；不宣称门牌/入口一致。详情营业时间仍直接展示原始 JSON，另行处理。

2026-09-07 真实数据边界：本轮 79 站仅覆盖 Toulouse 中心 12 km，列表现有上限 50；启动后不重新抓源，“刷新结果”不等于刷新官方快照。站名/营业及其他服务信息可能缺失，来源更新不等于现场验证。用户此前反馈定位显示 position disponible；本轮仅手动城市验证，不把该反馈当 GPS 精度审计。

2026-09-07 地图恢复验证：专用签名/自有受限密钥的独立包已实测显示 Toulouse 底图和两个 DEMO 标记，选中站点打开详情、返回和重开通过，旧无底图阻塞在本次独立包路径已解除。这不证明旧 Expo Go 空白的唯一根因，也不等同所有设备/网络/服务地图验收；生产结算/用量治理及真实导航仍待确认。

2026-09-07 地图后续：用户确认已保存应用/API 限制，本地新 APK 已含密钥并使用专用签名；以下早先“无密钥”记录为历史状态。旧模板签名包不能直接覆盖，新安装需用户明确同意仅测试应用的数据重置；SDK 实际鉴权和项目结算关联未验证，不扩大路线/Places 等服务范围。

2026-09-07 地图：本机原生构建/安装条件已解决，但自有 Google 地图密钥和结算配置未建立。当前无密钥的独立包保留地图禁用逻辑；尚不能通过独立包证明 Expo Go 空白问题的根因或修复。开发包使用模板调试签名，只用于无密钥本机冒烟；配置地图限制前须换专用测试签名，不把通用 debug 证书当安全限制，正式发布仍需独立发布签名。
...
- [x] 2026-09-07 导航 Linking 接收者修复：导航、来源页及证据署名链接改为通过对象调用 openURL；流程 mock 模拟原生 this 校验，新增 Android/iOS 失败→重试成功两项回归。全量格式、lint、类型、接口及 1028 tests 通过；华为既有真实站点详情按钮实测打开 Google Maps 路线预览，无需重装，无实际开始导航或新增云服务。手机保留预览，截图仅本机临时文件，见 [测试记录](./docs/development/local-real-fuel-testing.md)。

- [x] 2026-09-07 Toulouse 真实燃油本地手机测试：新增有界官方地区采集、隔离启动模式及三语范围/快照提示；导入 79 站并确认重放全跳过，API 50 条列表及对应 50 个详情通过，华为真实地址、地图标记及 Gazole 2,25 €/litre 与 API 一致。新增 8 tests，全量 1026 tests 通过。原 DEMO 临时库正常清理，原业务库不变；保留真实测试服务，未重装/清除手机数据、未启动外部导航。详见 [记录与复现](./docs/development/local-real-fuel-testing.md)。

- [x] 2026-09-07 华为独立地图真机有限冒烟：经明确授权仅卸载/替换 com.fuelnow.localtest，USB 8081/3001 转发恢复；手动 Toulouse / Carburant 返回 2 个 DEMO 结果，Google 底图、两个红色标记和署名可见，标记选中→Voir les détails→站点详情→Retour→重开地图通过。Expo Go 仍安装，测试包 coarse/fine 定位均 granted=false；当前应用近期有界日志样本无 FATAL EXCEPTION 或地图鉴权失败匹配。未进行真实定位/外部导航，未更改云端或启用其他 API；本轮仅测试与记录，不重复宣称单元测试或整体阶段验收。

- [x] 2026-09-07 受限地图密钥测试包准备：用户本地填写，秘密不进聊天/仓库；ARM64 原生构建成功，专用证书 v2/v3 验证通过，APK 内密钥与本地值安全比较一致，包名/权限核对通过。未安装、未卸载、未进行地图请求，等待测试数据重置授权；底图和 Phase 5/6 门槛仍未验收。

- [x] 2026-09-07 本地 Android 独立包准备及基础冒烟：Google 官方 Mac ARM 工具下载校验、所需 SDK 组件安装、ARM64 原生 APK 编译与 v2 签名校验完成；新增 test 独立身份及调试权限处理 6 tests，全量 1018 tests、原生配置隐私检查通过。最终 APK 移除模板悬浮窗及未使用 WorkManager 权限，保留网络/网络状态/前台定位和自身 signature 保护权限；USB 安装启动成功，手动 Toulouse / Fuel 最近搜索返回 2 个 DEMO 结果。无真实定位、导航、后台任务、云项目/密钥或计费操作；地图底图和阶段门槛未关闭。详见 [本地构建说明](./docs/development/android-local-build.md)。
...
2026-09-07 USB 开发连接修复：重启时实测 Metro 只监听 `[::1]:8081`，IPv4 健康请求失败；Expo CLI 的 localhost 绑定与当前 USB IPv4 转发不一致。测试子进程设置 IPv4-first DNS 后，实测监听 `127.0.0.1:8081` 且 HTTP 健康请求成功，重开 Expo 后收到 Android bundle 请求并编译成功。新增环境隔离回归，API 495 tests 通过。此项只证明本次本地连接恢复，不追溯断言所有旧 LAN 故障根因或长期稳定性。

2026-09-07 首页长词布局工程修复：测量服务卡片容器宽度，按字体缩放保守预留标题、勾选标识与内边距；窄屏/大字体单列、宽屏双列，无固定文字高度或禁用字体缩放。新增回归覆盖宽度/字号变化、选择保持和可访问性，全量 1003 tests 通过；华为视觉复验待进行。

2026-09-07 Fuel 详情工程修复：列表/地图跳转、详情加载/切换/重试与 API 契约保留并校验油品；真实临时库确认柴油 1.659 / SP95 E10 1.719，各按所选油品返回，无油品或无报价不猜测。新增 12 tests，全量 1002 tests、临时库 HTTP 检查及清理通过；独立提交 76215b9 已 push，华为复验待进行。

2026-09-07 华为测试字体恢复确认：用户明确反馈已恢复测试前字体档位，完成该设置恢复子项，未独立读取精确档位，不宣称整体设备设置或全部无障碍通过。已发现的 Fuel 详情上下文缺口、地图空白和 Combustible 词内断行继续保留，建议确认后进入修复与回归。文档差异检查通过，仅提交文字记录，未改应用代码、手机设置或停止测试服务。

2026-09-07 华为超大字体首页交互验收：用户对四服务入口能否点击选中、滚动后语言按钮能否打开分别回答“能”，完成这两个具体交互子项。Combustible 词内断行仍待优化，其他页面/读屏/完整无障碍未验收，恢复原字体档位尚未明确确认。文档差异检查通过，仅提交文字测试记录，未修改应用或设备设置。
