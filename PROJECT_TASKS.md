...
> 下一项任务：本地真机测试环境已实现，先确认 iPhone 15 Pro Max / 华为 Mate 20 的系统与 SDK 57 客户端、局域网连通性并实际验收；发布与真实 Beta 条件仍未关闭，见 [真机测试说明](./docs/development/local-phone-testing.md)
> 最后更新：2026-09-07
...
- [ ] `P5-REL-04` 完成核心用户流程回归测试
  ...
  - [x] `P5-REL-04b` 独立临时库、模拟数据、API/Expo 统一启动与停止/重置、三语测试提示及模拟导航保护；本机四服务/详情/价格与双平台开发包验证通过（2026-09-07；[真机测试说明](./docs/development/local-phone-testing.md)）；手机系统、安装及局域网端到端验收仍待实际结果
...
2026-09-07 真机：已验证本机服务/双平台编译及停机清理，私有网卡监听已建立，但本轮 Mac 工具访问 LAN 地址超时，回环 Metro 正常；是否系统本地网络许可/防火墙/隔离仍待手机浏览器检查，未关闭防火墙。SDK 57 客户端与设备系统版本也须确认；不能把电脑版自检当手机已连通。
