...
> 下一项任务：用户授权后已切换华为 USB 回环测试，旧 LAN 临时库已清理，原库未改；手机新地址 exp://127.0.0.1:8081 的启动日志已见。下一步确认 USB 画面、验收 Fuel Best 并观察是否再断连；Fuel 详情油品上下文仍待修复，原连接故障根因未确认。iPhone LAN 测试暂不可用，App 安装仍待处理；发布条件未关闭，见 [真机测试说明](./docs/development/local-phone-testing.md)
> 最后更新：2026-09-07
