...
> 当前状态：经用户授权，华为测试环境已切换到 Toulouse 中心 12 km 的 DGCCRF 真实燃油快照，共 79 站；列表最多 50 条、真实地图标记及柴油报价真机核对通过。全量 1026 tests 通过；未自动同步、未启用付费路线、未修改原业务库。Phase 5 为 10/21 主任务及 18 个子项；Phase 6 为 5/13 主任务及 8 个准备子项；未发布，小时任务保持暂停
> 当前阶段：本机真实燃油数据有限真机测试（单次区域快照，不是全国实时服务；Phase 5/6 均未整体验收）
> 下一项任务：等待用户体验真实列表/油品/地图的反馈，再安排刷新策略及剩余真机回归；真实站点外部导航、其他平台、长期 USB 稳定性、生产账单治理与发布门槛仍未关闭。保留当前测试服务，见 [真实燃油测试说明](./docs/development/local-real-fuel-testing.md)
> 最后更新：2026-09-07
