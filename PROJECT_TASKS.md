...
> 下一项任务：华为西班牙语首页即时切换已由照片确认，下一步检查 Toulouse / Combustible / Gasóleo 结果页本地化；首页 Combustible 单词内换行记录为待优化，不将首页通过等同全页面三语或持久化通过。地图持续空白及 Fuel 详情油品上下文仍待修复，原连接故障根因及 USB 长期稳定性未确认。iPhone LAN 测试暂不可用，App 安装仍待处理；发布条件未关闭，见 [真机测试说明](./docs/development/local-phone-testing.md)
> 最后更新：2026-09-07
