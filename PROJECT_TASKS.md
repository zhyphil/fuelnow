...
> 下一项任务：华为西班牙语详情顶部及 Air 卡片可见文字已由照片确认；下一步关闭并重新打开同一 USB 项目，检查西班牙语偏好是否保留，不清除数据或重装。首页 Combustible 单词内换行仍待优化，全页面三语及持久化未验收。地图持续空白及 Fuel 详情油品上下文仍待修复，原连接故障根因及 USB 长期稳定性未确认。iPhone LAN 测试暂不可用，App 安装仍待处理；发布条件未关闭，见 [真机测试说明](./docs/development/local-phone-testing.md)
> 最后更新：2026-09-07
