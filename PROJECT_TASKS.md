...
> 下一项任务：华为 Barcelona / Charge 最低价和当前营业禁用项实际点击保持最近排序，已由用户确认；下一步检查 Paris / Fuel 空结果提示，再返回 Toulouse 验证恢复（本套数据不覆盖 Paris，不依赖真实数据或地图密钥）。Fuel 详情油品上下文仍待修复，原连接故障根因及 USB 长期稳定性未确认。iPhone LAN 测试暂不可用，App 安装仍待处理；发布条件未关闭，见 [真机测试说明](./docs/development/local-phone-testing.md)
> 最后更新：2026-09-07
