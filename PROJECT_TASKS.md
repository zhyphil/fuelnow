...
> 当前阶段：本机真机测试准备（Phase 5/6 均未整体验收）
> 下一项任务：本地真机测试环境已实现，用户确认手机浏览器后端可达（具体设备待确认）；接着确认华为底层 Android 版本、匹配 SDK 57 的客户端安装及 App 内连接并实际验收；发布与真实 Beta 条件仍未关闭，见 [真机测试说明](./docs/development/local-phone-testing.md)
> 最后更新：2026-09-07
...
- [ ] `P5-REL-04` 完成核心用户流程回归测试
  ...
  - [x] `P5-REL-04b` 独立临时库、模拟数据、API/Expo 统一启动与停止/重置、三语测试提示及模拟导航保护；本机四服务/详情/价格与双平台开发包验证通过（2026-09-07；[真机测试说明](./docs/development/local-phone-testing.md)）；用户确认手机浏览器后端可达，具体设备、华为底层 Android 版本、客户端安装及 App 端到端验收仍待确认
...
| Beta 签名构建、设备和测试安排 | iPhone 15 Pro Max / iOS 26.6.1；华为 Mate 20 HMA-L29 / EMUI 12.0.0，底层 Android 版本待确认；签名客户端与验收待填 | QA-10f、LEG-03、REL-03/04 真机与人工验收 |
...
2026-09-07 真机：已验证本机服务/双平台编译及停机清理；Mac 工具访问 LAN 地址曾超时，回环 Metro 正常，随后用户确认手机浏览器可访问后端并看到 Toulouse / Barcelona / La Jonquera，未更改防火墙。具体手机归属仍待确认，不能标记两台均通过；后端浏览器检查也不能代替 Expo 8081、SDK 57 客户端安装及 App 功能验收。
...
2026-09-07 P5-REL-04b 连通性补充：用户确认手机浏览器后端可达并看到三个测试城市；设备归属未提供，逐设备验收留空，App 功能不标完成。环境提交 `17da613` 已推送，GitHub CI 通过；下一步是兼容客户端安装与 App 内验证。
