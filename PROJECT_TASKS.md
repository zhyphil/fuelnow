...
> 下一项任务：华为地图持续空白仍未通过，但用户确认 Retour 可返回原 2 个结果，返回子项通过；下一步继续不依赖地图的西班牙语即时切换测试，地图加载问题保留待查。Fuel 详情油品上下文仍待修复，原连接故障根因及 USB 长期稳定性未确认。iPhone LAN 测试暂不可用，App 安装仍待处理；发布条件未关闭，见 [真机测试说明](./docs/development/local-phone-testing.md)
> 最后更新：2026-09-07
