...
> 下一项任务：照片确认跟随设备语言已勾选且显示法语；只读设备语言列表与代码核对表明，中文之后的法语是首个受支持项。下一步检查重开后该模式是否仍勾选，不更改手机设置。首页 Combustible 单词内换行仍待优化，全页面三语及其他持久化场景未验收。地图持续空白及 Fuel 详情油品上下文仍待修复，原连接故障根因及 USB 长期稳定性未确认。iPhone LAN 测试暂不可用，App 安装仍待处理；发布条件未关闭，见 [真机测试说明](./docs/development/local-phone-testing.md)
> 最后更新：2026-09-07
