...
> 当前状态：Phase 5 已完成 10/21 主任务及 7 个工程子项；按用户要求暂停每小时自动任务，当前任务直接连续迭代；运营/部署/分发留待单独决定
> 当前阶段：Phase 5 测试、合规与发布准备
> 下一项任务：`P5-QA-10d` EV/OSM 开发来源链路；外部决定继续待办，见 [发布前置条件](./docs/testing/phase5-release-prerequisites.md)
> 最后更新：2026-09-07
...
- [ ] `P5-QA-10` 人工抽查真实站点、价格和营业状态 — 进行中：2026-09-07 已远程核对两国 10 个当前官方站点记录；未完成真实数据 API/原生端及人工验收，见 [部分证据](./docs/testing/phase5-live-source-audit.md)
  ...
  - [ ] `P5-QA-10d` 补齐获准开发的 EV/OSM 来源链路，严格保留许可证与动态能力限制
    ...
    - [ ] `P5-QA-10d2` 补充来源独立身份、事务写入及四服务 SQL 回归
    - [ ] `P5-QA-10d3` 官方 EV 快照有界读取、完整站点分组、更新/失败报告与原始内容摘要
    - [ ] `P5-QA-10d4` OSM 正面标签投影及开发导入，保留 ODbL 边界，不开启每次用户搜索的公共 Overpass 请求
...
- [ ] `P5-REL-05` 完成上线检查和回滚方案 — 进行中：清单检查器和运行手册完成，实际部署/备份恢复/回滚演练未完成
...
| 2026-09-07 | 正式采集到 canonical 业务数据的端到端链路未闭合，当前业务库为空，EV/OSM 正式接入仍有缺口                                              | 高     | 补齐采集、投影、调度与监控；真实来源经 API 和原生端验收后再关闭 QA-10/REL-04，测试 fixture 不计入生产覆盖                           | 工程待补齐，发布受阻                      |
