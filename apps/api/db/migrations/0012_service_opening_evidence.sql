...
ALTER TABLE service_point_services
  ADD COLUMN IF NOT EXISTS opening_hours jsonb,
  ADD COLUMN IF NOT EXISTS opening_status text NOT NULL DEFAULT 'unknown',
  ADD COLUMN IF NOT EXISTS opening_status_evaluated_at timestamptz;

ALTER TABLE service_point_services
  ADD CONSTRAINT service_point_services_opening_hours_shape_check
    CHECK (opening_hours IS NULL OR jsonb_typeof(opening_hours) = 'object'),
  ADD CONSTRAINT service_point_services_opening_status_check
    CHECK (
      opening_status IN (
        'open', 'closed', 'closing_soon', 'opening_soon', 'unknown'
      )
    ),
  ADD CONSTRAINT service_point_services_opening_status_time_check
...
VALUES ('0012_service_opening_evidence')
