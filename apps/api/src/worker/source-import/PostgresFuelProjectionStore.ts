...
import { projectFuelSource, type FuelSourceProjection } from "./projectFuelSource.js";
...
export class PostgresFuelProjectionStore {
  ...
  public async persist(
    ...
    try {
      ...
      for (const { source, projection } of entries) {
        ...
        const existing = await client.query(
          ...
          [point.id, JSON.stringify(projection), JSON.stringify(source.record)],
        );
  ...
  private async writePoint(
    client: PoolClient,
    { point, air, wash }: FuelSourceProjection,
  ) {
