...
| 华为 Mate 20 HMA-L29 | EMUI 12.0.0；底层 Android 未显示，Expo Go 已安装、版本待填 | 通过（2026-09-07，用户确认） | Expo Go 安装、项目启动通过（用户确认） | 部分：Toulouse 列表及多服务详情、FR 文案显示；Fuel 详情油品上下文缺失待修，其余待测 | | DEMO 禁用样式/提示已见；实际点击及真实地图导航待测 |
