...
- [ ] 首页长词排版：此前照片中选中燃油卡标题 Combustible 显示为 Combust / ible 两行；本次超大字体照片中未选中卡片仍显示为 Combus / tible。未见字母丢失，但单词内断行影响可读性，不限于选中态，保留窄屏/大字体自适应待优化。原字号档位未提供，不推断准确缩放比例；测试后恢复个人原设置不等于修复，不以缩小字体规避问题。
...
- [ ] 超大字体首页交互：保持当前超大字体，依次点击四个服务入口，确认均能显示对应选中状态；向下滚动，确认 Idioma · Español 可以打开语言弹窗。无需重新搜索或启用本地测试事件；操作反馈与显示证据分开记录。
- [ ] 测试设置恢复：完成上述点击检查后，将字体大小恢复用户记下的原档位，粗细保持标准，不改显示缩放/分辨率；当前尚未确认已恢复。一次超大字体首页检查不代替全部页面、其他字体档位或读屏验收。
