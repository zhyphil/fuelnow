...
2026-09-07。用户确认：iPhone 15 Pro Max / iOS 26.6.1；华为 Mate 20 HMA-L29 / EMUI 12.0.0。EMUI 版本不能直接当底层 Android 版本，后者仍待确认。客户端安装和实际手机连接仍待确认；这不是正式 Beta 发布或现场验收。
...
工程验证：全量 990 tests（新增 17）通过；真实临时库四服务/详情/新鲜模拟油价通过；通过回环地址获取的 iOS/Android Expo manifest 与开发包编译通过；Ctrl+C 停止和重启清理已实测，前两次临时库已删除，原库站点数仍为 0。私有网卡监听已建立，但本轮 Mac 工具访问 LAN 地址超时，手机连通性仍待用户检查，未更改防火墙。
