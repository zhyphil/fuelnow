...
2026-09-07。iPhone 15 Pro Max / iOS 26.6.1；华为 Mate 20 HMA-L29 / EMUI 12.0.0，USB 读取确认 Android 10 / API 29、Expo Go 57.0.9。两台手机 LAN 浏览器连通已通过；华为四服务基础展示、Fuel 基础排序、Charge 禁用项点击、DEMO 列表/详情导航保护、空结果与列表恢复已有分项证据，详见下方记录。西班牙语首页即时切换已确认；Combustible 卡片标题单词内换行待优化，全页面语言与偏好持久化未验收。地图持续空白未通过，但可返回原来的 2 个结果；Fuel 详情油品上下文缺口仍待修复。当前为华为 USB 回环环境，LAN 测试暂停；下一步检查西班牙语燃油结果页，USB 长期稳定性仍待确认。这不是正式 Beta 发布或现场验收。
...
- [ ] 西班牙语燃油结果页：保持 Español，使用 Toulouse / Combustible / Gasóleo 搜索，核对结果标题、数量、排序和价格/未知状态文字；仅首页切换不能证明结果页完整本地化。地图空白与 Fuel 详情缺口继续保留待查。
