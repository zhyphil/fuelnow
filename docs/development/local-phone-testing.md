...
2026-09-07。iPhone 15 Pro Max / iOS 26.6.1；华为 Mate 20 HMA-L29 / EMUI 12.0.0，USB 读取确认 Android 10 / API 29、Expo Go 57.0.9。两台手机 LAN 浏览器连通已通过；华为四服务基础展示、Fuel 基础排序、Charge 禁用项点击、DEMO 列表/详情导航保护、空结果与列表恢复已有分项证据，详见下方记录。西班牙语可见页面、重开保留和改选英语后的重开保留已确认；跟随设备语言选项已勾选并显示法语，与设备语言列表的首个受支持项一致。Combustible 首页标题单词内换行待优化，全页面语言和其他偏好场景未验收。地图持续空白未通过，但可返回原来的 2 个结果；Fuel 详情油品上下文缺口仍待修复。当前为华为 USB 回环环境，LAN 测试暂停；下一步检查重开后仍保留跟随设备语言模式，USB 长期稳定性仍待确认。这不是正式 Beta 发布或现场验收。
...
- [ ] 跟随系统模式跨重开保留：关闭语言弹窗，再关闭并重开同一项目，重新打开语言弹窗但不点语言选项，确认仍勾选 Utiliser la langue de l’appareil 而非手动 Français/English。同样显示法语不足以证明模式保留，需确认勾选项；系统语言变化后的动态跟随仍另行待验，不要求现在改手机设置。
