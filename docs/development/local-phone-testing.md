...
| 华为 Mate 20 HMA-L29 | EMUI 12.0.0；Android 10 / API 29；Expo Go 57.0.9（USB 读取确认） | 通过（2026-09-07，用户确认 3001 和 8081/status） | 安装/启动已通过；中途加载失败后恢复，稳定性待查 | 部分：Toulouse 列表及多服务详情、FR 文案显示；Fuel 详情油品上下文缺失待修，其余待测 | | DEMO 禁用样式/提示已见；实际点击及真实地图导航待测 |
...
- [ ] 先在 Toulouse 的 Fuel 列表展开 `Tri et filtres`，选择 `Gazole`，确认列表报价，再测试价格排序；与详情缺口分开记录。
