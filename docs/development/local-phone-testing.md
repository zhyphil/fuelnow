...
## 当前状态：真实燃油 USB 测试（2026-09-07）

当前华为独立 Fuel Now Test 已切换到 **Toulouse 中心 12 km、79 个官方真实站点**，不是下方历史 DEMO 环境。真实地图和所选柴油价格已核对。启动、快照限制及恢复 DEMO 请以 [真实燃油测试说明](./local-real-fuel-testing.md) 为准；不要同时启动第二份服务。下方为早先分阶段测试记录，其中 Expo Go 空白、旧工具位置和 DEMO 描述不代表当前独立包状态。
