...
2026-09-07。iPhone 15 Pro Max / iOS 26.6.1；华为 Mate 20 HMA-L29 / EMUI 12.0.0，USB 读取确认 Android 10 / API 29、Expo Go 57.0.9。两台手机 LAN 浏览器连通已通过；华为四服务基础展示、Fuel 基础排序、Charge 禁用项点击、DEMO 列表/详情导航保护、空结果与列表恢复已有分项证据，详见下方记录。西班牙语可见页面、重开保留和改选英语后的重开保留已确认；跟随设备语言显示法语符合匹配规则，用户确认重开后系统模式仍勾选。Combustible 首页标题单词内换行待优化，全页面语言和其他偏好场景未验收。地图持续空白未通过，但可返回原来的 2 个结果；Fuel 详情油品上下文缺口仍待修复。当前为华为 USB 回环环境，LAN 测试暂停；下一步检查较大字体下西班牙语首页布局，USB 长期稳定性仍待确认。这不是正式 Beta 发布或现场验收。
...
- [ ] 较大字体首页检查：延续 Phase 5 真机大字体验收，使用西班牙语首页观察已知 Combustible 单词内换行及四服务卡。请用户记下系统字体原档位，仅把字体调大一档（若已最大则保留并说明），返回 App 核对文字是否被截断/重叠、四入口及滚动是否可用；不同时改显示缩放或分辨率，以便对照。测试后恢复原档位。一次首页检查不代替全部页面、大字边界或读屏验收；当前尚未调设置或取得结果。
