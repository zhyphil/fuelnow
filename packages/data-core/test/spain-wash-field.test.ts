...
describe("Spain Wash source-field boundary", () => {
  it("keeps Wash unknown across every committed real Spain record", async () => {
    const { records, sourceSnapshotAt } = await loadUniqueRecords();
    const fieldNames = [...new Set(records.flatMap((record) => Object.keys(record)))];
    const washPattern =
      /lavado|lavacoch|wash|limpieza|aspir|vacuum|t[uú]nel|rodillo|presi[oó]n/i;

    expect(records).toHaveLength(684);
    expect(fieldNames).toHaveLength(41);
    expect(fieldNames.filter((field) => washPattern.test(field))).toEqual([]);
  ...
  it("does not confuse customer-service modes with a Wash facility", async () => {
    const { records, sourceSnapshotAt } = await loadUniqueRecords();
